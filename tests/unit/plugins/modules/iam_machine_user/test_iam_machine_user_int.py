...
@pytest.fixture
def unique_machine_user_name() -> str:
    """Generate a unique machine user name for each test to avoid conflicts in parallel execution."""
    return f"test-machine-user-int-{uuid.uuid4().hex[:8]}"
...
def test_iam_machine_user_roles_purge(
    ...
    assert result.value.changed is True
    assert result.value.machine_user["machine_user_name"] == unique_machine_user_name
    assert len(result.value.machine_user["roles"]) == 3

    # Purge roles - replace with only one role
    module_args(
        {
            "endpoint": BASE_URL,
            "access_key": ACCESS_KEY,
    ...
    # Verify old roles are removed
    assert (
        "crn:altus:iam:us-west-1:altus:role:BillingAdmin"
        not in result.value.machine_user["roles"]
    )
    assert (
        "crn:altus:iam:us-west-1:altus:role:ClassicClustersCreator"
        not in result.value.machine_user["roles"]
